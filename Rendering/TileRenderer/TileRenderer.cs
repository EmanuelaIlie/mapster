...
public static class TileRenderer
{
    public static BaseShape Tessellate(this MapFeatureData feature, ref BoundingBox boundingBox, ref PriorityQueue<BaseShape, int> shapes)
    {
        ...
        if (feature.Properties.Any(p => p.Key == TerrainTypes.highway && MapFeature.HighwayTypes.Any(v => p.Value.StartsWith(v))))
        {
        ...
        // Am folosit data din enum in loc de string
        else if (feature.Properties.Any(p => p.Key == TerrainTypes.boundary && p.Value.StartsWith("forest")))
        {
            var coordinates = feature.Coordinates;
        ...
        // Am folosit data din enum in loc de string
        else if (feature.Properties.Any(p => p.Key == TerrainTypes.landuse && (p.Value.StartsWith("forest") || p.Value.StartsWith("orchard"))))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Forest);
            baseShape = geoFeature;
        ...
        // Am folosit data din enum in loc de string
            p => p.Key == TerrainTypes.landuse && (p.Value.StartsWith("residential") || p.Value.StartsWith("cemetery") || p.Value.StartsWith("industrial") || p.Value.StartsWith("commercial") ||
        ...
        // Am folosit data din enum in loc de string
            p => p.Key == TerrainTypes.landuse && (p.Value.StartsWith("farm") || p.Value.StartsWith("meadow") || p.Value.StartsWith("grass") || p.Value.StartsWith("greenfield") ||
        ...
        // Am folosit data din enum in loc de string
                 ...
                 feature.Properties.Any(p => p.Key == TerrainTypes.landuse && (p.Value.StartsWith("reservoir") || p.Value.StartsWith("basin"))))
        {
            var coordinates = feature.Coordinates;
        ...
        // Am folosit data din enum in loc de string
        else if (feature.Type == GeometryType.Polygon && feature.Properties.Any(p => p.Key == TerrainTypes.building))
        {
            var coordinates = feature.Coordinates;
            var geoFeature = new GeoFeature(coordinates, GeoFeature.GeoFeatureType.Residential);
            baseShape = geoFeature;
